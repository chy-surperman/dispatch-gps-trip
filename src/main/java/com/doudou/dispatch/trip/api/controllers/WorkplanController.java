...
import com.doudou.dispatch.trip.api.services.impl.DBWorkplanServiceImpl;
...
public class WorkplanController {
    ...
    @Autowired
    private DBWorkplanServiceImpl dbWorkplanService;
...
//        List<GpsLocus> gpsLocusList = new ArrayList<>();
    ...
    @RequestMapping("/submitWorkplanSave")
    public JsonResult submitWorkplanSave(@RequestHeader("Authorization") String userName,
                                     @RequestBody WorkplanChangeRecode changeRecord) throws ParseException {
        JsonResult jsonResult = new JsonResult();
        if(StringUtils.isEmpty(userName)){
            return jsonResult.setCode(HttpApiConstans.not_login);
        }
        ...
        JsonResult jsonResult1 = gpsFeignClient.getVehicleLocusMapGps(workplan.getDateString()+" "+workplan.getDepartureString(), workplan.getDateString()+" "+workplan.getArrivalString(), workplan.getVehicleId(), 1, "gpsDB");
        List<GpsLocus> gpsLoci = JSON.parseArray(JSON.toJSONString(jsonResult1.getResult()), GpsLocus.class);
        if (gpsLoci.isEmpty()){
            workplan.setMileage(0);
        }else{
            workplan.setMileage(dbWorkplanService.calMileage(gpsLoci));
        }
        try {
            workplan.setDate(DateUtil.getDate(workplan.getDateString(), DateStyle.YYYY_MM_DD));
            workplan.setScheduleTime(DateUtil.getDate(workplan.getDepartureString(), DateStyle.HH_MM_SS));
